...
<?php endif; ?>
	<div id="filter-bar" class="btn-toolbar">
		...
		<div class="btn-group pull-left">
			...
			<button class="btn hasTooltip" type="button" title="<?php echo Text::_('JSEARCH_FILTER_CLEAR'); ?>" onclick="document.id('filter_search').value='';this.form.submit();">
	...
	<table class="table table-striped" id="projectList">
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
				...
				<td class="center">
					<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'projects.', $canChange, 'cb'); ?>
				</td>
				<td class="nowrap has-context">
					<div class="pull-left">
						...
						<?php if ($canEdit) : ?>
						<a href="<?php echo Route::_('index.php?option=com_advportfolio&task=project.edit&id=' . (int) $item->id); ?>">
					...
					<?php
						...
						HTMLHelper::_('dropdown.edit', $item->id, 'project.');

						HTMLHelper::_('dropdown.divider');
						if ($item->state) :
							HTMLHelper::_('dropdown.unpublish', 'cb' . $i, 'projects.');
						else :
							HTMLHelper::_('dropdown.publish', 'cb' . $i, 'projects.');
						endif;

						HTMLHelper::_('dropdown.divider');
						if ($archived) :
							HTMLHelper::_('dropdown.unarchive', 'cb' . $i, 'projects.');
						else :
							HTMLHelper::_('dropdown.archive', 'cb' . $i, 'projects.');
						endif;

						if ($item->checked_out) :
							HTMLHelper::_('dropdown.checkin', 'cb' . $i, 'projects.');
						endif;

						if ($trashed) :
							HTMLHelper::_('dropdown.untrash', 'cb' . $i, 'projects.');
						else :
							HTMLHelper::_('dropdown.trash', 'cb' . $i, 'projects.');
						endif;
