 ...
 */
class AdvPortfolioTableProject extends JTable {
	...
	public function store($updateNulls = false) {
		...
		$table = JTable::getInstance('Project', 'AdvPortfolioTable');
		if ($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0)) {
			$this->setError(JText::_('COM_ADVPORTFOLIO_PROJECT_ERROR_UNIQUE_ALIAS'));
	...
	public function check() {
		...
		if (trim($this->title) == '') {
			$this->setError(JText::_('COM_ADVPORTFOLIO_PROJECT_ERROR_TABLES_TITLE'));
		...
		if ($xid && $xid != intval($this->id)) {
			$this->setError(JText::_('COM_ADVPORTFOLIO_PROJECT_ERROR_TABLES_NAME'));
		...
		if ($this->type == 1 && !preg_match($youtube_regex, $this->video_link) && !preg_match($vimeo_regex, $this->video_link)) {
			$this->setError(JText::_('COM_ADVPORTFOLIO_PROJECT_ERROR_VIDEO_LINK'));
